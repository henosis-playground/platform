...
export const object = defineResource<KubernetesObject, Record<never, never>>({
  kind: "k8s/object@1",
  outputs: {},
});
...
): EmittedResource<Record<never, never>> {
 ...
 */
export function emitServicePair(
  context: BuildContext,
  name: string,
  spec: ServicePairSpec,
): { readonly host: string; readonly port: number; readonly url: string } {
...
function assertDnsLabel(name: string, label: string): void {
