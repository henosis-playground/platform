...
type MaterialisedBinding = Resolved<BindingShape>;
...
for (const name of input.order) {
  ...
  const rawBinding = executeBinding(component);
  bindings[name] = materialiseBinding(rawBinding, info.envId);
  ...
  if (info.isRender) {
    resources[name] = executeBuild(component, {
      ...
      envId: info.envId,
      depResolver: <T extends BindingShape>(
        depComponent: Component<T>,
      ): Resolved<T> => {
        ...
        return materialiseBinding(executeBinding(depComponent), depInfo.envId);
      },
    });
  }
}
...
async function importComponent(
  ...
  if (!isRecord(imported) || !isComponentLike(imported.default)) {
